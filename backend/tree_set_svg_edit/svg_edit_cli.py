...
def italicize_genus_species(svg_file, output_file):
    ...
    for text_elem in root.findall(f'.//{{{SVG_NS}}}text'):
        ...
        # Detectar scale bar value (números decimais como "0.02", "0.1")
        # Valores de suporte são inteiros (50, 95, 100), então não têm ponto
        if re.match(r'^0?\.\d+$', text) or re.match(r'^\d+\.\d+$', text):
            # É um valor decimal (scale bar) - aplicar estilo específico
            ...
            # Estilo igual aos tip labels (fino)
            text_elem.set('style', 'font-size:15px;font-family:Helvetica, Arial, sans-serif')
            text_elem.text = text
            continue
